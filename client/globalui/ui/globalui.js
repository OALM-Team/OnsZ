function appendNewCard(hexa, title, content, timeout) {
    var container = document.getElementById("container");
    var div = document.createElement("div")
    div.classList.add("card")
    div.classList.add("animated")
    div.classList.add("zoomIn")
    div.innerHTML = '<div class="icon-container"><img class="info-icon" src="./info.png" /></div>' +
    '<div style="display: inline-block; margin-left: 10px;">' +
        '<div class="title" style="color: ' + hexa + '">' + title +'</div>' +
        '<div class="content">' +
            content +
        '</div>' +
    '</div>' +
    ...
    var img = div.querySelector(".icon-container");
    const rgb = hexToRgb(hexa);
    const color = new Color(rgb[0], rgb[1], rgb[2]);
    const solver = new Solver(color);
    const result = solver.solve();
    $(img).css("filter", result.filter)

    div.querySelector(".timeout-bar-inner").style.backgroundColor = hexa;
    $(div.querySelector(".timeout-bar-inner")).animate({
    width: "0%"
    }, timeout, () => {
